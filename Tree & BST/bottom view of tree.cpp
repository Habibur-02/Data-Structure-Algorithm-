 ...
 void postOrderTraverse(node* root)
 {
  ...
  bool subtree(node* root, node* root1)
  {
      if(root==NULL || root1== NULL)
        return root==root1;
      if(root->data==root1->data && is_identical(root, root1))
      {
          return true;
      }
      
      bool left= subtree(root->left, root1);
      bool right= subtree(root->right, root1);
      return left || right;
  }
  ...
  void topview(node* root)
  {
      ...
      while(q.size()>0)
      {
          ...
          q.pop();
              ...
              m[distance]= curNode->data; // ekhane change sudhu top view er code er hisebe
          
          
          if(curNode->left != NULL)
          {
              q.push({curNode->left, distance - 1});
          }
          
          if(curNode->right!= NULL)
          {
              q.push({curNode->right, distance + 1});
          }
...
int main()
{
    ...
    // cout<<sum_node(root)<<'\n';
    vector<int> PreOrderSequence1= {1,20,-1,-1,3,4,-1,-1,5, -1, -1};
